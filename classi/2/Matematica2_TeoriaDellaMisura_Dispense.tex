...
\begin{tikzpicture}[scale=0.85]
    ...
    \coordinate (Ap) at (4, 6);
    \coordinate (Bp) at (6, 3);
    \coordinate (Cp) at (8, 0);
    ...
    % Trasversali r, s (non parallele: si incontrano in V)
    \draw[thick] (V) -- (C); \node[above left] at (V) {$V$};
    \draw[thick] (V) -- (Cp);
    \node[left] at ($(A)!0.4!(C)$) {$r$};
    \node[right] at ($(Ap)!0.4!(Cp)$) {$s$};

    \node[circle, fill, inner sep=1.3pt] at (V) {};
    \node[circle, fill, inner sep=1.3pt] at (A) {}; \node[left] at (A) {$A$};
    \node[circle, fill, inner sep=1.3pt] at (B) {}; \node[left] at (B) {$B$};
    \node[circle, fill, inner sep=1.3pt] at (C) {}; \node[left] at (C) {$C$};
    \node[circle, fill, inner sep=1.3pt] at (Ap) {}; \node[right] at (Ap) {$A'$};
    \node[circle, fill, inner sep=1.3pt] at (Bp) {}; \node[right] at (Bp) {$B'$};
    \node[circle, fill, inner sep=1.3pt] at (Cp) {}; \node[right] at (Cp) {$C'$};
\end{tikzpicture}
\end{center}
...
\paragraph{Un lemma.} Dimostriamo il teorema per due sole rette del fascio, ad esempio $a$ e $b$: mostriamo cioè che
...
\begin{tikzpicture}[scale=0.9]
    \coordinate (V) at (0, 4);
    \coordinate (B) at (2, 0);
    \coordinate (Bp) at (6, 1);
    \coordinate (A) at (1, 2);
    \coordinate (Ap) at (3, 2.5);
...
\begin{enumerate}
\item I triangoli $AA'B$ e $AA'B'$ hanno la stessa base $AA'$ e i vertici $B,B'$ equidistanti dalla retta $a$ (perché $B, B'$ appartengono entrambi alla retta $b$, parallela ad $a$): hanno quindi la stessa area,
$$\text{Area}(AA'B) = \text{Area}(AA'B')$$
\item I triangoli $VAA'$ e $BAA'$ hanno lo stesso vertice $A'$ e basi $VA$, $AB$ sulla stessa retta $r$: le loro aree stanno quindi come le basi,
$$\frac{\text{Area}(VAA')}{\text{Area}(BAA')} = \frac{\overline{VA}}{\overline{AB}}$$
\item Analogamente, i triangoli $VA'A$ e $B'A'A$ hanno lo stesso vertice $A$ e basi $VA'$, $A'B'$ sulla stessa retta $s$:
...
Poniamo $k = \dfrac{\overline{VA}}{\overline{AB}} = \dfrac{\overline{VA'}}{\overline{A'B'}}$, cioè $\overline{VA}=k\cdot\overline{AB}$ e $\overline{VA'}=k\cdot\overline{A'B'}$. Allora
$$\overline{VB}=\overline{VA}+\overline{AB}=(k+1)\overline{AB}, \qquad \overline{VB'}=\overline{VA'}+\overline{A'B'}=(k+1)\overline{A'B'}$$
e quindi
...
\paragraph{Conclusione della dimostrazione.} Applichiamo il lemma appena dimostrato:
\begin{itemize}
    \item alla coppia di rette $a,b$: si ottiene $\overline{VA}:\overline{VB} = \overline{VA'}:\overline{VB'}$; poniamo $k$ il valore comune di questo rapporto;
    \item alla coppia di rette $b,c$ (lo stesso ragionamento, applicato questa volta a $b,c$ anziché $a,b$): si ottiene $\overline{VB}:\overline{VC} = \overline{VB'}:\overline{VC'}$; poniamo $h$ il valore comune di questo rapporto.
\end{itemize}
...
$$\overline{AB}=\overline{VB}-\overline{VA}=h(1-k)\,\overline{VC}, \qquad \overline{BC}=\overline{VC}-\overline{VB}=(1-h)\,\overline{VC}$$
$$\overline{A'B'}=\overline{VB'}-\overline{VA'}=h(1-k)\,\overline{VC'}, \qquad \overline{B'C'}=\overline{VC'}-\overline{VB'}=(1-h)\,\overline{VC'}$$
Quindi
